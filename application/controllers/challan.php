...
class Challan extends CI_Controller {
    ...
    public function add_challan() {
      ...
      echo'<pre>';
         var_dump($challan);
      echo'</pre>';
       // $insert_into_challan = $this->orders->insertData('tblt_challan', $challan);
         
         $insert_into_challan=1;
        
          foreach ($sku_id as $key => $value) {
            $challan_line = array(
                'challan_id' => $insert_into_challan,
                'sku_id' => $value,
                'price' => $TP_price[$key],
...
//            /var_dump($challan_line);            
          //  $insert_into_challan_line = $this->orders->insertData('tblt_challan_line', $challan_line);
        }
